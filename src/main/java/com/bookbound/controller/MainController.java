...
public class MainController {
     ...
     * Injects user data and reading statistics.
     */
    @GetMapping("/home")
    public String home(@RequestParam(required = false) String userId, Model model) {
        if (userId != null) {
            try {
                ...
                // Add recent books and popular genres for dashboard
    ...
    public String search(@RequestParam(required = false) String query,
        ...
        // Add metadata for search filters
    ...
    public String bookDetail(@PathVariable String bookId, 
        ...
        try {
            ...
            if (bookOptional.isPresent()) {
                ...
                // If user is logged in, check if they're tracking this book
                if (userId != null) {
                    // This would require a method to check tracking status
                    model.addAttribute("userId", userId);
                }
                
                // Add related books from same series or genre
    ...
    public String library(@RequestParam(required = false) String userId, Model model) {
        if (userId != null) {
            try {
                ...
                // This would require TrackingService integration
                // var trackingList = trackingService.getUserTrackingList(userId);
